...
def get_yaml_from_key(bucket: str, key: str) -> str:
    global S3_RESOURCE
    try:
        ...
        if isinstance(node, dict):
            ...
            if func_name:
                ...
                if base_name in file_map:
                    ...
                    content_str = get_yaml_from_key(bucket_name, file_key)
...
class B2YamlInterpreter(ExecutionEnvironment):
    language: str = "yaml"
    ...
    def __init__(
        ...
        self.files = [f for f in list_files(bucket_name, prefix) if f.endswith(".yaml")]
        self.file_map = {}
        for file_key in self.files:
            base_name = os.path.splitext(os.path.basename(file_key))[0]
            if base_name in self.file_map:
                logger.warning(f"Duplicate base name '{base_name}' for file '{file_key}'")
            self.file_map[base_name] = file_key
        self.storage = {}
    ...
    def parse_error_logs(self, execution_logs: str) -> str:
        ...
        capture = False
        for line in lines:
            if 'yaml.errors' in line or 'InterpreterError' in line:
                error_lines.append(line.strip())
                capture = True
            elif capture and (line.startswith('File ') or line.strip() == ''):
                capture = False
            elif capture:
                error_lines.append(line.strip())
    ...
    def inject_yaml_file_list(self, code_action: str) -> str:
    ...
    def parse_code_blobs(self, code_blob: str) -> str:
        ...
        if not matches:
            try:
                yaml.safe_load(code_blob)
                return code_blob
            except yaml.YAMLError as e:
                error_msg = self.parse_error_logs(str(e))
                raise ValueError(f"Invalid YAML:\n{error_msg}\n\nEnsure code is enclosed in ```yaml...```")
    ...
    def get_final_storage(self) -> str:
        ...
        content = get_yaml_from_key(self.b2_config["bucket_name"], self.storage[max_step])
        return content if content else ""

    def get_storage(self, next_step_id: int) -> str:
        if next_step_id not in self.storage:
            return ""
        content = get_yaml_from_key(self.b2_config["bucket_name"], self.storage[next_step_id])
        return content if content else ""