...
class ActionStep(MemoryStep):
    ...
    def dict(self):
        ...
        return {
            ...
            "function_calls": (
                [tc.dict() for tc in self.function_calls] if self.function_calls else []
            ),
            ...
            messages.append(
                Message(
                    ...
                    content=[
                        {
                            "type": "text",
                            "text": str([tc.dict() for tc in self.function_calls]),
                        }
                    ],
                )
            )
...
@dataclass
class VisionStep(MemoryStep):

    model_input_messages: List[ChatMessage]

    def __init__(self, model_input_messages: List[ChatMessage], message: ChatMessage):
        self.model_input_messages = model_input_messages
        self.message = message
        super().__init__()

    def get_markdown_table(self) -> pd.DataFrame:
        ...
        for line in lines:
            if md_pattern.match(
                line.strip()
            ):  # Check if the line is a markdown table row
                markdown_lines.append(line.strip())

        # Initialize an empty DataFrame
        dataframe = pd.DataFrame()

        # Proceed only if there are enough lines for header, separator, and data
        if len(markdown_lines) >= 2:
            # Extract headers from the first line
            headers = [
                cell.strip() for cell in markdown_lines[0].split("|") if cell.strip()
            ]

            # Skip the separator line (markdown_lines[1]) and process data lines
            data_lines = markdown_lines[2:]
            data = []
            for line in data_lines:
                cells = [cell.strip() for cell in line.split("|") if cell.strip()]
                # Only add rows that match the number of headers
                if len(cells) == len(headers):
                    data.append(cells)

            # Create DataFrame if there's valid data
            if headers and data:
...
class PlanningStep(MemoryStep):
    ...
    def __init__(self, facts: str, plan: str):
        ...
        self._plan_status = {
            step_id: "step" for step_id, step in enumerate(self._plan_list)
        }
        super().__init__()
    ...
    def to_messages(self, summary_mode: bool, **kwargs) -> List[Dict[str, str]]:
        ...
        if not summary_mode:
            messages.append(
                Message(
                    role=MessageRole.ASSISTANT,
                    content=[{"type": "text", "text": self.facts.strip()}],
                )
            )
        messages.append(
            Message(
...
class JudgeStep(MemoryStep):
    ...
    def get_feedback_content(self) -> str:
        ...
        content = (
            content.replace("#approve", "").replace("#fail", "").replace("#rethink", "")
        )
        return content
...
class StepsList(Generic[T]):
    ...
    def save_step_in_memory(
        self, task_step: Union[TaskStep, ActionStep, PlanningStep]
    ) -> None:
        content = None
        step_type = None
...
class AgentMemory:
    def __init__(self, name: str, company_name: str, system_prompt: str = ""):
        ...
        self.steps: StepsList[Union[TaskStep, ActionStep, PlanningStep]] = StepsList(
            name
        )
    ...
    def append_step(self, step: Union[TaskStep, ActionStep, PlanningStep, VisionStep]):
